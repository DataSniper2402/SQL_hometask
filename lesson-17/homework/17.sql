...
select products.product_name,sum(unit) from orders join products on orders.product_id=products.producrt_id
...
with recursive degrouped as ( select Product, Quantity, 1 as qty from Grouped where Quantity > 0 union all
 select Product, Quantity, qty + 1 from degrouped where qty + 1 <= Quantity)
select Product from degrouped order by Product;
